...
def train_model(num_layer=2):
    ...
    if num_layer == 2:
        model.save(str(root_path / 'models' /'Model.bif'))
    else:
        model.save(str(root_path / 'models' /'SingleLayerModel.bif'))
...
def residual_plot(match_index, plot_type='both', scaling=10., index_scope=None, noise_compare=False):
    ...
    model_path = root_path / 'models' /'SingleLayerModel.bif'

    model = BayesianNetwork.load(str(model_path))
    ...
    if plot_type in ('predicted', 'both'):
        ...
        for point_index in range(start_index, test_match.length + end_index):
            ...
            predicted_effect = ((predict_match_time(inference, test_match, point_index)[0]) * 2 - 1) * scaling

            weighted_effect = predicted_effect
        ...
        plt.plot(X, Y_sg, label='Predicted Effect', color="palegreen", linewidth=3)
    ...
    if plot_type in ('residual', 'both'):
        ...
        # if plot smoother curve, edit window length to be 31
        Y_res_sg = savgol_filter(Y_res, 31, 3)  # Apply Savitzky-Golay filter for smoothing
        if noise_compare:
            plt.plot(X_res, Y_res, label='Raw Residual Effect', color=light_green)
        plt.plot(X_res, Y_res_sg, label='Denoised Residual Effect', color="orange", linewidth=3)

    if plot_type in ('both',):
        # Assuming Y_sg and Y_res_sg are your smoothed predicted and actual effects respectively,
        # and X and X_res are the corresponding X values (they need to be the same for fill_between to work correctly).

        # Ensure both X arrays are the same, this might require adjusting based on how your data is structured:
        X_common = np.linspace(start_index, start_index + len(Y_sg) - 1, num=len(Y_sg))

        # Fill the area between the curves
        plt.fill_between(X_common, Y_sg, Y_res_sg, color='grey', alpha=0.2)  # Adjust 'alpha' for transparency
...
if __name__ == "__main__":
    pass
    # residual_plot(-1, plot_type="residual", noise_compare=True)
